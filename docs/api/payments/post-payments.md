# API: Ghi nhận một khoản thu, mặc định ở trạng thái chờ xác nhận
...
- **Purpose:** Ghi nhận một khoản thu, mặc định ở trạng thái chờ xác nhận.
- **Method:** `POST`
- **Endpoint:** `/payments`
...
| amount | number \| string | Có | ≥ 0.0; mẫu `^(?!^[-+.]*$)[+-]?0*\d*\.?\d*$` |
